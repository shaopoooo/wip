...
- [x] 分頁缺 total count：已全面實作 server-side 分頁（`Paged<T>`），`countCol` + `pagedResult`，所有 admin list API 回傳 `{ items, total, page, limit, totalPages }`
...
**1. 看板基本顯示**
- 開啟 `http://localhost:5173/dashboard`
- 確認頁面正常載入，顯示站點卡片
- 確認頂部有部門 tab（全廠 / A 線 / B 線）及右上有更新時間

**2. 部門篩選**
- 點「A 線」→ 確認只顯示 A 線站點，今日統計只計 A 線
- 點「全廠」→ 確認全部站點恢復顯示

**3. 站內 WIP（in_station 模式）**
- 確認初始為「站內 WIP」模式
- 用掃描 PWA 對 `WO-FPC-2026-001` 做 check-in（選 A 線站點）
- 回到看板手動點「↺」刷新
- 確認對應站點卡片數字變為 1（綠色）
- 用掃描 PWA 完成 check-out 後刷新看板，確認數字歸 0（灰色）

**4. 待入站（queuing 模式）**
- 切換為「待入站」
- 對已完成第 1 站 check-out 的工單查看看板
- 確認第 2 站卡片出現數字（該工單正在等待進入第 2 站）

**5. 今日統計 banner**
- 確認頂部顯示「今日：X 完工、X 出站、X 生產中」
- 完成一張工單所有站後，刷新確認「完工」數字遞增
...
**8. 追溯查詢 — 基本流程**
- 開啟 `http://localhost:5173/trace`
- 輸入 `WO-FPC-2026-001` → 點查詢
- 確認顯示工單基本資訊（料號、數量、部門、狀態）
- 確認顯示「站點歷程」tab 及時間軸

**9. 追溯 — 時間軸內容**
- 確認各站顯示入站時間（台灣時區，格式 M/D HH:MM）
- 已出站的站確認顯示出站時間及在站時長
- 自動補填站確認有半透明樣式及「自動補填」標籤
- 進行中的站確認顯示藍色「● 進行中」

**10. 追溯 — 母子單關聯樹**
- 先用管理後台對 `WO-FPC-2026-001` 執行拆單（M6 完成後可測；目前可跳過）
- 若有子單：確認「母子單 (N)」tab 出現
- 點 tab 確認顯示樹狀結構，目前工單標示「← 目前」
- 點其他節點確認畫面切換並重新查詢該工單歷程

**11. 追溯 — 錯誤處理**
- 輸入不存在的工單號 → 確認顯示紅色錯誤提示

#### Backend — 看板 + 追溯 API
- [x] `GET /api/dashboard/wip?department_id=&mode=`（mode: `in_station` or `queuing`）
- [x] `GET /api/dashboard/today?department_id=`（當日產出統計）
- [x] `GET /api/dashboard/work-order-progress?department_id=`
- [x] `GET /api/dashboard/station/:stationId/work-orders?mode=`（站點工單列表，含 in_station / queuing）
- [x] `GET /api/traceability/:workOrderId`（完整站點歷程；亦接受 orderNumber）
- [x] `GET /api/traceability/:workOrderId/family`（母子單關聯樹）

#### Frontend — 看板（`/dashboard`）
- [x] 部門切換 tab（A 線 / B 線 / 全廠）
- [x] 各站 WIP 數量卡片（顏色：0=灰, 1~3=綠, 4~7=黃, 8+=紅脈動）
- [x] WIP 模式切換（站內 / 待入站）
- [x] 當日產出統計（大卡片：今日完工、今日出站、生產中工單）
- [x] 兩欄佈局（左側 WIP 站點、右側工單進度列表）
- [x] WIP=0 group 自動折疊為摘要行，有 WIP 的展開顯示卡片
- [x] 「顯示空站 / 隱藏空站」toggle
- [x] 站點卡片點擊展開站內工單列表（drill-down）
- [x] 工單進度列表（組別 + 站數基準，點擊跳 trace）
- [x] 30 秒 polling 自動刷新

#### Frontend — 追溯（`/trace`）
- [x] 輸入工單號（或掃 QR Code）
- [x] 時間軸顯示各站 check-in → check-out（台灣時區）
- [x] 母子單關聯樹（可展開 / 收合）
...
#### Backend
- [x] 新增 `product_categories` 表（migration `0003`）
- [x] `products` 新增 `category_id`、`route_id` 欄位（1:1 路由，不記版本）
- [x] `work_orders` 新增 `order_qty`（訂單需求數量）欄位
- [x] `GET /api/admin/product-categories` CRUD router
- [x] Seed 修正：所有訂單歸 A 線（軟板）部門；軟板/軟硬結合板改為產品種類
- [x] `GET /api/products` 回傳 `categoryId`, `categoryName`, `routeId`
- [x] `POST /api/admin/work-orders` 支援 `orderQty` 欄位

#### Frontend
- [x] 首頁（`/`）改為 DashboardPage（issue 9）
- [x] Dashboard 全廠視圖：改為按部門分組，各部門顯示獨立 header（issue 10）
- [x] 工單進度列表點擊 → 新分頁 `/trace?wo=<orderNumber>`（issue 11）
- [x] TracePage：讀取 `?wo=` 參數自動觸發查詢（issue 11）
- [x] 列印頁（`/admin/print`）移至無 sidebar 的 layout（issue 2）
- [x] ProductsPage + RoutesPage 合併為單一頁面（兩個 tab）（issue 3）
- [x] 新增 CategoriesPage（`/admin/categories`）（issue 5）
- [x] 工單詳情頁顯示預計製程步驟 + 進度 overlay（issue 1）
- [x] 工單建立 Modal 新增訂單需求數量欄位（issue 7）
- [x] 建立工單時依所選產品自動填入路由（issue 3/4）
- [x] 所有管理頁面加入模糊搜尋 + 分頁（issue 6）：Groups, Stations, Equipment, Roles, Users, WorkOrders, Products, Categories（後升級為 server-side，見 M4.2）

---

### 🔧 M4.2 — 製程路由模板 + Server-side 分頁

#### Backend — 製程路由模板
- [x] `process_routes` 新增 `is_template` (boolean) + `template_type` (varchar 50) 欄位
- [x] migration `0006_route_templates.sql`
- [x] `GET /api/admin/process-routes` 全面支援 server-side 分頁（`parsePage` + `pagedResult`）
- [x] `DELETE /api/admin/process-routes/:id` 阻擋模板停用（回錯 `VALIDATION_ERROR`）
- [x] `POST /api/admin/process-routes/:id/clone` 從模板複製新路由（含所有步驟）
- [x] `POST /api/admin/products` + `PATCH` 阻擋直接指派模板路由給料號
- [x] Seed 新增 4 種製程模板：單面板 / 雙面板 / 多層板 / 軟硬結合板（各含完整工序步驟）

#### Backend — 通用查詢工具
- [x] `backend/src/utils/queryHelpers.ts`：`parsePage`, `buildOrder`, `searchCond`, `pagedResult`, `countCol`
- [x] 所有 admin list API 改用 queryHelpers（Customers, Vendors, Users, Roles, Categories, Stations, Groups, Equipment, Products, Routes, WorkOrders, DeviceTokens）

#### Frontend — useServerTable + TableControls
- [x] `frontend/src/hooks/useServerTable.ts`：memoized `params` 物件、300ms 搜尋 debounce、toggleSort
- [x] `frontend/src/components/TableControls.tsx`：搜尋框 + 每頁筆數 + 分頁列
- [x] `frontend/src/api/admin.ts`：`Paged<T>` 型別、`buildTableQuery()` helper、所有 list API 回傳 `Paged<T>`、新增 `listAll()` 供下拉選單
- [x] 所有 12 個管理頁面改用 `useServerTable` pattern（server-side 分頁/搜尋/排序）
- [x] RoutesPage：模板區塊顯示 4 張彩色卡片，支援「套用模板」（CloneModal）+ 「管理步驟」（StepsModal）
