...
# Docker（根目錄）
docker compose up -d --build                                                            # 開發（自動合併 override.yml，ENV_FILE 預設 .env.dev）
docker compose -f docker-compose.yml -f docker-compose.prod.yml up -d            # 生產（ENV_FILE 預設 .env）
ENV_FILE=.env.staging docker compose -f docker-compose.yml -f docker-compose.prod.yml up -d  # 其他環境
...
> | `.env` | production | ❌ |
> | `.env.dev` | development | ❌ |
> | `.env.example` | production 範本 | ✅ |
> | `.env.dev.example` | development 範本 | ✅ |
>
...
> - 正式環境 / CI：**一律使用 `npm ci`**，禁止 `npm install`
> - 新增套件前確認發佈日期：`npm view <package> time --json | head -20`
...
- 看板（WIP 堆積、工單進度、母單匯總）
- 基礎追溯（工單級歷程）
- **管理後台 JWT 認證**（僅 `/admin` 頁面與 `/api/admin/*` 需要登入，其餘頁面開放）
...
| `/trace` 追溯查詢 | ❌ 開放 |
| `/admin/*` 管理後台 | ✅ JWT 登入 |
| `POST /api/scan` | ❌ 開放（用 device_id 識別）|
| `GET /api/dashboard/*` | ❌ 開放 |
| `GET /api/traceability/*` | ❌ 開放 |
| `POST/PATCH/DELETE /api/admin/*` | ✅ JWT |
...
| 操作員辨識 | Phase 1 不做，只記設備端（掃描側） |
| 登入認證 | /admin 做 JWT；掃描 / 看板 / 追溯不做 |
| RBAC | Phase 1 支援角色 CRUD（可新增/刪除角色），但不做細粒度權限；Phase 2 升級 |
